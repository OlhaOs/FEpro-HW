...
(function ($) {
  $.fn.ie8Gallery = function (options, callback) {
    ...
    base.events = function () {
        ...
        $(settings.thumb).each(function (i) {
          if ($(this).attr('data-img-src') == currentImageSource) {
            ...
            $(settings.underlay)
              ...
              .animate({ opacity: 0.6 }, 250, 'linear', function () {
                $(settings.overlay)
                  .css({ visibility: 'visible' })
                  .animate({ opacity: 1 }, 250, 'linear');
              });

            $(settings.galleryImage + ' img').attr(
              'src',
      ...
      $(document).on(
        'click',
        settings.galleryImage + ',' + settings.underlay,
        function (e) {
          e.preventDefault();
          $(settings.underlay + ',' + settings.overlay).animate(
            { opacity: 0 },
            250,
            'linear',
            function () {
              $(settings.underlay + ',' + settings.overlay).css({
                visibility: 'hidden',
              });
            }
          );
      ...
      $(document).on('click', settings.btnNext, function (e) {
        e.preventDefault();
        currentImageIndex =
          ++currentImageIndex > images.length - 1 ? 0 : currentImageIndex;
        $(settings.galleryImage + ' img').attr(
          'src',
          images[currentImageIndex]
        );
